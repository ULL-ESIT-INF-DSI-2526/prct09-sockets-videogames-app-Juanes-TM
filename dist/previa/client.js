...
yargs(hideBin(process.argv))
    .command("add", "Añade un videojuego", {
    ...
    const juego = new VideoJuego(argv.id, argv.name, argv.desc, argv.platform, argv.genre, argv.developer, argv.year, argv.multiplayer, argv.hours, argv.value);
    ...
    .command("read", "Muestra un videojuego", {
    user: { type: "string", demandOption: true },
    id: { type: "number", demandOption: true },
}, (argv) => {
    const client = net.connect({ port: 60300 });
    ...
    .command("list", "Muestra una lista de videojuegos", {
    user: { type: "string", demandOption: true },
}, (argv) => {
    const client = net.connect({ port: 60300 });
    client.write(JSON.stringify({
    ...
    .command("update", "Actualiza un videojuego", {
    user: { type: "string", demandOption: true },
    id: { type: "number", demandOption: true },
    name: { type: "string" },
    desc: { type: "string" },
    ...
    if (argv.name !== undefined)
        cambios.nombre = argv.name;
    if (argv.desc !== undefined)
        cambios.descripcion = argv.desc;
    if (argv.platform !== undefined)
        cambios.plataforma = argv.platform;
    if (argv.genre !== undefined)
        cambios.genero = argv.genre;
    if (argv.developer !== undefined)
        cambios.desarrolladora = argv.developer;
    if (argv.year !== undefined)
        cambios.lanzamiento = argv.year;
    if (argv.multiplayer !== undefined)
        cambios.multijugador = argv.multiplayer;
    if (argv.hours !== undefined)
        cambios.horasEstimadas = argv.hours;
    if (argv.value !== undefined)
        cambios.valorMercado = argv.value;
    client.write(JSON.stringify({
        type: "update",
        user: argv.user,
        id: argv.id,
